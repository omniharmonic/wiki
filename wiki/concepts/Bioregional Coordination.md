---
title: "Bioregional Coordination"
type: concept
visibility: public
aliases:
  - "Bioregional Coordination: Sacred Work in a Time Between Worlds"
  - "place-based organizing"
  - "watershed governance"
  - "bioregional commons"
related:
  - "[[wiki/concepts/bioregionalism]]"
  - "[[wiki/concepts/Knowledge Commons]]"
  - "[[wiki/concepts/commons-governance]]"
  - "[[wiki/concepts/polycentric-governance]]"
  - "[[wiki/concepts/Knowledge Organizational Infrastructure]]"
  - "[[wiki/concepts/Cascadia Bioregional Movement]]"
  - "[[bioregional-coordination-sacred-work]]"
  - "[[the-infrastructure-of-belonging]]"
tags:
  - governance
  - bioregional
  - commons
  - place-based
created: "2026-06-06"
updated: "2026-06-06"
---

# Bioregional Coordination

## Definition

Bioregional Coordination is the practice of aligning human governance with living systems at the scale of a bioregion — weaving traditional institutions together with emergent network commons so that decisions are made by many centers rather than a single hierarchy. Benjamin Life frames it as "sacred work in a time between worlds": the threshold between empire's collapse and the emergence of place-based governance attuned to the land. It is less a fixed institution than an ongoing way of organizing — coordination without ownership, structure without domination.

## Learning from the Forest

Benjamin's foundational move is to learn coordination from ecology itself. Mycorrhizal networks coordinate vast forests without hierarchy, ownership, or a central controller — nutrients and signals flow through relationships, not command. This becomes a model for [[wiki/concepts/polycentric-governance|polycentric governance]]: many centers of decision-making operating like organs in a body, each autonomous yet serving the whole. He anchors the practice in Elinor Ostrom's eight design principles for enduring commons, treating the bioregion as a shared resource to be stewarded rather than enclosed.

## Composting Empire

Bioregional Coordination is explicitly post-colonial work. Benjamin traces the "colonial DNA" of modern governance to the joint-stock corporation (1602) and the Treaty of Westphalia (1648) — instruments of ownership and bordered sovereignty that fragmented our relationship to place. Rather than rejecting these tools outright, he describes "composting empire": using the materials of the dying order to build something new. A key practice is **institutional self-negation**, where backbone nonprofits explicitly acknowledge they serve rather than govern the bioregion — Regenerate Cascadia being his exemplar. The goal is to embed bioregional governance daily in every organization one touches, not only at annual congresses (drawing on David Henke and the 1980s bioregional movement).

## Knowledge Commons at Scale

Coordination across many bioregions requires shared knowledge without imposed standardization. Federated [[wiki/concepts/Knowledge Commons|knowledge commons]] — connected via the [[wiki/concepts/Knowledge Organizational Infrastructure|Knowledge Organizational Infrastructure]] protocol — let agents federate place-based knowledge across bioregions while preserving local difference. This is the infrastructural layer of the [[wiki/concepts/the-great-turning|Great Turning]] (Joanna Macy): an ontological shift from separation to [[wiki/concepts/interbeing|interbeing]], honoring indigenous relationship to land while building genuinely post-colonial governance. Named collaborators include the Commoning Standard and MetaGov; pilots include Spirit of the Front Range and the [[wiki/concepts/Cascadia Bioregional Movement|Cascadia Bioregional Movement]], with the OpenCivics consortium as the connecting infrastructure.

## In Benjamin's Work

This concept is developed most fully in the essay [[bioregional-coordination-sacred-work|Bioregional Coordination: Sacred Work in a Time Between Worlds]] (presented to the r3.0 Bioregional Confluence). It connects closely to [[the-infrastructure-of-belonging|The Infrastructure of Belonging]] and the broader argument for place-based parallel societies in [[collapse-parallel-societies-bioregional-imperative|the bioregional imperative]].
